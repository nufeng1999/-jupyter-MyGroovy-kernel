...
class GroovyKernel(MyKernel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wAll = True # show all warnings by default
        self.wError = False # but keep comipiling for warnings
    def compile_with_groovyc(self, source_filename, binary_filepath=None, cflags=None, ldflags=None,env=None,coptions=None):
    ...
    def _exec_groovyc_(self,source_filename,magics):
        ...
        if p.returncode != 0:  # Compilation failed
            ...
            self._logln("Groovyc exited with code {}, the executable will not be executed".format(p.returncode),3)
    ...
    def do_runcode(self,return_code,fil_ename,class_filename,outpath,magics,code, silent, store_history=True,
        ...
        if magics['st']['runmode']=='repl':
            self._start_replprg('java',magics['st']['joptions']+[class_filename] + magics['st']['args'],magics)
        ...
        # #################
        # else:
        cmdstr = ['java']+magics['st']['joptions']+[class_filename] + magics['st']['args']
        self._log(' '.join((' '+ str(s) for s in cmdstr))+"\n")
        p = self.create_jupyter_subprocess(['java']+magics['st']['joptions']+[class_filename] + magics['st']['args'],env=self.addkey2dict(magics,'env'),magics=magics)
        self.subprocess=p
        self.g_rtsps[str(p.pid)]=p
        return_code=p.returncode
        # bcancel_exec,retstr=self.raise_plugin(code,magics,return_code,fil_ename,3,2)
