...
##//%include:src/comm_attribute.py
    def __init__(self, *args, **kwargs):
        ...
        self.kernelinfo='[MyGroovyKernel]'
...
#################
    ...
    def _exec_groovyc_(self,source_filename,magics):
        ...
        if returncode != 0:  # Compilation failed
            ...
            self._logln("Groovyc exited with code {}, the executable will not be executed".format(returncode),3)
...
##do_runcode
    def do_runcode(self,return_code,fil_ename,class_filename,outpath,magics,code, silent, store_history=True,
        ...
        if magics['_st']['runmode']=='repl':
            self._start_replprg('java',magics['_st']['joptions']+[class_filename] + magics['_st']['args'],magics)
            return_code=p.returncode
            # bcancel_exec,retstr=self.raise_plugin(code,magics,return_code,fil_ename,3,2)
            return bcancel_exec,retinfo,magics, code,fil_ename,retstr
        ############################################
    ############################################
        ...
        p = self.create_jupyter_subprocess(['java']+magics['_st']['joptions']+[class_filename] + magics['_st']['args'],env=self.addkey2dict(magics,'env'),magics=magics)
        self.subprocess=p
        self.g_rtsps[str(p.pid)]=p
        return_code=p.returncode
        ##代码启动后
